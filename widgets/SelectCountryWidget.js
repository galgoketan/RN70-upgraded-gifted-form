...
import {
  View, Text, TouchableHighlight, TextInput, Image, PixelRatio, AppState, Appearance
} from 'react-native';


var ListView = require('deprecated-react-native-listview')
...
module.exports = createReactClass({
  ...
  onSelect(name, value) {
    ...
    this.setState({
      dataSource: new ListView.DataSource({rowHasChanged: (r1, r2) => r1 !== r2}).cloneWithRows(countries)
    })}
    this._onChange(name);
    ...
    if (image === null) {
      return (
        <TouchableHighlight
          key={rowData.alpha2}
          onPress={() => {
            this.onSelect(rowData.name, rowData[this.props.code])}}
          underlayColor={this.getStyle('underlayColor').pop()}

          // style={selectedCountry[rowData.name]?this.getStyle(['selectedRow']):this.getStyle(['row'])}
          style={[this.getStyle(['row']), {backgroundColor: this.state.scheme === 'dark' ? 'rgb(44, 43, 43)' : '#FFF'}]}
        >
          <View style={{
            flexDirection: 'row',
            alignItems: 'center',
      ...
      return (
        <TouchableHighlight
          ...
          style={[this.getStyle(['row']), (this.state.country === rowData.name) ? {backgroundColor: '#c7c7cc'} : {backgroundColor: this.state.scheme === 'dark' ? 'rgb(44, 43, 43)' : '#FFF'}]}
        >
          <View style={{
            flexDirection: 'row',
            alignItems: 'center',
            ...
            <Text numberOfLines={1} style={[{
              flex: 1
            },this.state.country===rowData.name?{color:'black'}:{color: this.state.scheme === 'dark' ? '#FFF' : 'black'}]}>{rowData.name}</Text>
          </View>
        </TouchableHighlight>
      );
    }
  ...
  renderHeader() {
    return (
      <View
        style={[this.getStyle(['textInputContainer']), {backgroundColor: this.state.scheme === 'dark' ? 'rgb(44, 43, 43)' : '#C9C9CE'}]}
      >
        <TextInput
          autoFocus={this.props.autoFocus}

          style={[this.getStyle(['textInput']), {backgroundColor: this.state.scheme === 'dark' ? 'rgb(44, 43, 43)' : '#FFFFFF'}]}
